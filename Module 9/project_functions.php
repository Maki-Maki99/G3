...
function assignTaskToEmployee($taskData) {
    ...
    $status = isset($employee['status']) ? $employee['status'] : '';
    $availability = isset($employee['availability']) ? $employee['availability'] : '';

    if (strtolower($status) === 'on leave' || strtolower($availability) === 'on leave' || strtolower($status) === 'terminated') {
    ...
    // Passed HR check — now insert task (safe prepared stmt)
    global $conn;
    ...
    $task_code = 'T' . time(); // simple code; adjust as needed
...
function checkProjectBudget($project_id) {
    ...
    // Expect keys: allocated_budget, used_budget, remaining_budget
